...
def fetch_news_for_stock(stock: str):
    now = datetime.now()
    if stock in stock_news_cache:
        cached_data, timestamp = stock_news_cache[stock]
    ...
    if not compound_scores:
        ...
        stock_news_cache[stock] = (result, now)
        return result
...
def get_news(request: NewsRequest):
    return fetch_news_for_stock(request.stock)
...
def get_watchlist():
    ...
    for stock in WATCHLIST_STOCKS:
        results.append(fetch_news_for_stock(stock))
