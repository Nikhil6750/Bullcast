...
export function getSupabasePersistenceDiagnostic() {
  return {
    ...
    lastReturnedRowIds: lastStorageDiagnostic?.lastReturnedRowIds ?? [],
...
function logPersistenceStatus(status) {
  ...
  const payload = {
    ...
    lastReturnedRowIds: status.lastReturnedRowIds,
...
function createStorageResult({
  ...
  const status = {
    ...
    lastReturnedRowIds: isSave
      ? returnedRowIds ?? safeArray(trades).map((trade) => trade?.id).filter(Boolean)
      : lastStorageDiagnostic?.lastReturnedRowIds ?? [],
...
export async function saveJournalTradesToStorage(trades, options = {}) {
  ...
  try {
    ...
    return createStorageResult({
      ...
      returnedRowIds: savedRows.map((row) => row?.id).filter(Boolean),
...
export async function saveJournalTradeToStorage(trade, options = {}) {
  ...
  try {
    ...
    return createStorageResult({
      ...
      rowsSaved: 1,
      returnedRowCount: 1,
      returnedRowIds: [savedTrade.id].filter(Boolean),
